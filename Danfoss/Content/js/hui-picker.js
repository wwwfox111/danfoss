...
function huiPicker(selector, callBack) {
	...
	huiPickerMain.innerHTML = '<div class="hui-picker-menu">' + '<div class="hui-fl hui-button hui-button-small" style="color:#999999;" onclick="huiPickerHide();">取消</div>' + '<div class="hui-fr hui-button hui-button-small hui-primary" id="HUI_PickerConfirm' + HUI_PickerId + '">确定</div>' + '</div>' + '<div class="hui-picker-list-in"></div>' + '<div class="hui-picker-line"></div>';
	...
	this.pickerBtn.click(function () {
		hui('.hui-picker').hide();thisObj.pickerMain.show();
	});
	...
	this.scrollFun = function () {
		if (HUI_PickerTimer != null) {
			clearTimeout(HUI_PickerTimer);
		}
		var scTop = this.scrollTop,
		    scObj = this;
		HUI_PickerTimer = setTimeout(function () {
			thisObj.scrollDo(scTop, scObj);
		}, 200);
	};
	this.scrollDo = function (scTop, scObj) {
		scObj.removeEventListener('scroll', this.scrollFun);
		var cList = hui(scObj),
		    index = Math.round(scTop / 30),
		    oldIndex = scObj.getAttribute('huiseindex');
		...
		if (levelNumber < this.level - 1 && thisObj.relevance) {
			if (oldIndex != index) {
				this.nextReBind(index, levelNumber + 1);
			}
		}
		setTimeout(function () {
			scObj.addEventListener('scroll', thisObj.scrollFun);
		}, 300);
	};
