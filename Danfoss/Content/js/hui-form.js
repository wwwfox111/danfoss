...
hui.formTextClear = function () {
	...
	for (var i = 0; i < textClears.length; i++) {
		textClears[i].onkeyup = function () {
			...
			HuiInputsClear.onclick = function () {
				thisObj.value = '';this.style.display = 'none';
			};
		};
	}
};
...
hui.eyesChange = function (o) {
	var _selfDom = hui(o),
	    _inputDom = _selfDom.parent().find('.hui-input').eq(0);
...
function huiFormCheck(selector) {
	...
	var inputs = formIn.find('input'),
	    selects = formIn.find('select'),
	    textareas = formIn.find('textarea');res = true;
	for (var i = 0; i < inputs.dom.length; i++) {
		res = huiFormCheckBase(inputs.dom[i]);if (!res) {
			break;
		}
	}
	if (res) {
		for (var i = 0; i < selects.dom.length; i++) {
			res = huiFormCheckBase(selects.dom[i]);if (!res) {
				break;
			}
		}
	}
	if (res) {
		for (var i = 0; i < textareas.dom.length; i++) {
			res = huiFormCheckBase(textareas.dom[i]);if (!res) {
				break;
			}
		}
	}
	if (typeof huiFormCheckAttach != 'undefined' && res) {
		if (!huiFormCheckAttach()) {
			return false;
		}
	}
	return res;
	...
	switch (checkType) {
		case 'string':
			...
			if (!reg.test(checkVal)) {
				return huiFormCheckShowErrMsg(checkMsg);
			}
			break;
		case 'int':
			...
			if (reg2.test(checkVal)) {
				return huiFormCheckShowErrMsg(checkMsg);
			}
			break;
		case 'between':
			if (!huiFormCheckNumber(checkVal, checkData, checkMsg)) {
				return false;
			}
			break;
		case 'betweenD':
		...
		case 'betweenF':
			...
			break;
		case 'same':
			if (checkVal != checkData) {
				return huiFormCheckShowErrMsg(checkMsg);
			}
			break;
		case 'sameWithId':
			if (checkVal != hui('#' + checkData).val()) {
				return huiFormCheckShowErrMsg(checkMsg);
			}
			break;
		case 'notSame':
...
function huiFormCheckNumber(checkVal, checkData, checkMsg) {
	checkVal = Number(checkVal);if (isNaN(checkVal)) {
		return huiFormCheckShowErrMsg(checkMsg);
	}
	checkDataArray = checkData.split(',');
	if (checkDataArray[0] == '') {
		if (checkVal > Number(checkDataArray[1])) {
			return huiFormCheckShowErrMsg(checkMsg);
		}
	} else if (checkDataArray[1] == '') {
		if (checkVal < Number(checkDataArray[0])) {
			return huiFormCheckShowErrMsg(checkMsg);
		}
	} else {
...
hui.getInputs = function (elementObj, method) {
	...
	if (method == 'textarea' || method == 'select') {
