...
var hui = function (selector) {
	...
	/* 选择器 */
	var _hui = function _hui(selector) {
		if (!selector) {
			selector = document;
		}
	...
	var hcExtends = {
		...
		val: function val(vars) {
			...
			try {
				...
				console.log(_lang.domEmpty);return null;
			}
		},
		...
		attr: function attr(attrName, val) {
			if (val) {
		...
		setAttr: function setAttr(attrName, val) {
			for (var i = 0; i < this.length; i++) {
		...
		/* 样式操作 */
		css: function css(csses) {
			for (var i = 0; i < this.length; i++) {
				var styles = this.dom[i].style;for (var k in csses) {
					styles[k] = csses[k];
				}
			}
			return this;
		...
		focusIn: function focusIn(callBack) {
			for (var i = 0; i < this.length; i++) {
				this.dom[i].addEventListener('focus', callBack);
			}
		},
		focusOut: function focusOut(callBack) {
			for (var i = 0; i < this.length; i++) {
				this.dom[i].addEventListener('focusout', callBack);
			}
		},
		on: function on(eventType, sonSelector, callBack) {
			...
			for (var i = 0; i < _huiDeputes.length; i++) {
				...
				for (var ii = 0; ii < objs.length; ii++) {
					if (objs.dom[ii] === e.target) {
						objs.dom[ii].index = ii;_huiDeputes[i].cb(objs.dom[ii]);break;
					}
				}
			}
		},
		longTap: function longTap(callBack) {
			if (this.length < 1) {
				return;
			}
			var timer = null,
		...
		swipeLeft: function swipeLeft(callBack) {
			if (this.length != 1) {
				return;
			}
			if (this.swiper == null) {
		...
		swipeRight: function swipeRight(callBack) {
			if (this.length != 1) {
				return;
			}
			if (this.swiper == null) {
		...
		swipeUp: function swipeUp(callBack) {
			if (this.length != 1) {
				return;
			}
			if (this.swiper == null) {
		...
		swipeDown: function swipeDown(callBack) {
			if (this.swiper == null) {
				this.swiper = new Hammer(this.dom[0]);
			}
			this.swiper.get('swipe').set({ direction: Hammer.DIRECTION_ALL });
		...
		swipeEnd: function swipeEnd(callBack) {
			this.swiper.on("panend", function (ev) {
				callBack(ev);
			});
		},
		trigger: function trigger(eventType, eventData) {
		...
		each: function each(callBack) {
			...
			for (var i = 0; i < this.length; i++) {
				this.dom[i].index = i;callBack(this.dom[i]);
			}
		},
		/* 筛选器 */
		...
		siblings: function siblings() {
			...
			while (nextNode = currentNode.nextElementSibling) {
				nodes.push(nextNode);currentNode = nextNode;
			}
			currentNode = startNode;
			while (preNode = currentNode.previousElementSibling) {
				nodes.push(preNode);currentNode = preNode;
			}
		...
		index: function index() {
			...
			while (preNode = startNode.previousElementSibling) {
				nodes.push(preNode);startNode = preNode;
			}
			return nodes.length;
		},
		...
		appendTo: function appendTo(parentObj) {
			if ((typeof parentObj === "undefined" ? "undefined" : _typeof(parentObj)) == 'object') {
				for (var i = 0; i < this.length; i++) {
		...
		prependTo: function prependTo(parentObj) {
			if ((typeof parentObj === "undefined" ? "undefined" : _typeof(parentObj)) == 'object') {
				for (var i = 0; i < this.length; i++) {
		...
		/* 宽、高获取 */
		height: function height(isOffset) {
			if (this.length != 1) {
				return 0;
			}
			if (isOffset) {
		...
		/* loadding button */
		loadingButton: function loadingButton(loadingText, isIcon) {
			if (!loadingText) {
				loadingText = 'Loading...';
			}if (!isIcon) {
				isIcon = true;
		...
		resetLoadingButton: function resetLoadingButton() {
			this.html(this.getAttr('HUI_BTN_RESET'));this.removeAttr('HUI_BTN_RESET');
		},
		buttonIsLoading: function buttonIsLoading() {
			if (this.attr('HUI_BTN_RESET')) {
				return true;
			}return false;
		},
		/* point msg */
		pointMsg: function pointMsg(msg, color, size, top, right, isRelative) {
			if (this.length < 1) {
				return false;
			}if (!isRelative) {
				isRelative = true;
			...
			if (!msg) {
				...
				var HUI_RedPoint = this.find('.hui-point-msg');if (HUI_RedPoint.length >= 1) {
					return;
				}
		...
		removePointMsg: function removePointMsg() {
			if (this.length < 1) {
				return false;
			}
		...
		/* switch box */
		switchBox: function switchBox(butNames, callBack) {
			if (!butNames) {
				butNames = ['Off', 'On'];
			}
		...
		getSwitchVal: function getSwitchVal() {
			if (this.hasClass('hui-switch-on')) {
				return true;
			}
			return false;
		},
		progressBar: function progressBar(val) {
			this.find('span').first().css({ width: val + '%' });
		},
		/* ranging */
		ranging: function ranging(callBack) {
		...
		rangeRuling: function rangeRuling() {
			for (var i = 0; i < this.length; i++) {
				var html = '';
		...
		scrollX: function scrollX(num, sonsTag, extraValue) {
			if (this.length < 1) {
				return;
			}if (!num) {
				num = 3;
		...
		scrollY: function scrollY(height) {
			if (this.length < 1) {
				return;
			}if (!height) {
				height = 150;
		...
		unfold: function unfold(height, text) {
			if (this.length < 1) {
				return;
			}if (!height) {
				height = 580;
	...
	_hui.offset = function (e) {
		...
		while (e = e.offsetParent) {
			offset.top += e.offsetTop;offset.left += e.offsetLeft;
		}return offset;
	};
	...
	_hui.toast = function (msg, timer) {
		...
		if (typeof plus != 'undefined') {
			plus.nativeUI.toast(msg, { duration: timer });return;
		}
	...
	_hui.alert = function (msg, btnName, callBack) {
		...
		btn.onclick = function () {
			hui.dialogClose();if (hui.dialogCallBack) {
				hui.dialogCallBack();
			}
		};
	};
	_hui.confirm = function (msg, btnName, callBack, callBack2) {
		...
		btns[0].onclick = function () {
			hui.dialogClose();if (callBack2) {
				callBack2();
			}
		};
		btns[1].onclick = function () {
			hui.dialogClose();if (callBack) {
				callBack();
			}
		};
	};
	...
	_hui.h5Loading = function (isClose, title, options) {
		if (isClose) {
			plus.nativeUI.closeWaiting();return;
		}
		if (!title) {
			title = '';
		};if (!options) {
	...
	_hui.blackMask = function () {
		...
		if (_hui.blackMasker.length < 1) {
			hui.toast('请在HTML中创建#hui-black-mask');return false;
		}
	...
	_hui.scrollNews = function (domId, speed) {
		...
		setInterval(function () {
			...
			setTimeout(function () {
				firstNews.removeClass('hui-scroll-news-h0');firstNews.appendTo(dom);
			}, 800);
		}, speed);
	};
	...
	_hui.create = function (winName, styles, isClose, extras) {
		...
		var w = plus.webview.getWebviewById(winName);if (w) {
			return w;
		}
	...
	_hui.drag = function (prevView, nextView, callBack) {
		...
		if (nextView) {
			...
			currentView.drag({ direction: "left", moveMode: "followFinger" }, { view: _next, moveMode: "follow" }, function (e) {
		...
		if (prevView) {
			...
			currentView.drag({ direction: "right", moveMode: "followFinger" }, { view: _prev, moveMode: "follow" }, function (e) {
				if (e.type == 'end' && e.result) {
	...
	_hui.close = function (vId) {
		var w = plus.webview.getWebviewById(vId);if (w) {
			w.close();
		}
	};
